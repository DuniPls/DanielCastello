    ...
    for i in range(66, 91, 1):
        ...
        for j in numpy.arange(1.5, 10.5, 0.5):
            ...
            for k in range(1, 11, 1):
                test = input.replace(".csv", "_%sHZ.csv" % str(str(1/k * 100).replace(".", ",")))
                run_forest(test)
    print("--- End of experiment 2.3 ---")
...
def run_forest_with_test(INPUT_PATH, TEST_PATH):
    ...
    """
    print("Loading file :: ", INPUT_PATH)
    f=open("22_METRICS.txt", "a+")
    g=open("22_MATRICES.txt", "a+")
    f.write(INPUT_PATH + "\n")
    g.write(INPUT_PATH + "\n")
    # Load the csv file into pandas dataframe
    dataset = pd.read_csv(INPUT_PATH)
