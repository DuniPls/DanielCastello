...
if __name__ == "__main__":
    ...
    dataset_folder = (os.path.dirname(os.path.abspath(__file__))) +\
            "/Datasets/"
    dataset = dataset_folder.split('Datasets/',1)[1]
    print(dataset_folder)
    #csvs_we_are_looking_for = ["back", "thigh", "label"]  # ["LOWERBACK", "THIGH", "labels"]
    #csvs_we_are_looking_for = ["", "index", "x_back", "y_back", "z_back", "x_thigh", "y_thigh", "z_thigh", "label", "subject"]
    csvs_we_are_looking_for = ["hunt4training.h5"]
    ...
    for r, ds, fs in os.walk(dataset_folder):
        ...
        if False not in found_csvs:
            ...
            subject_files.append(found_csvs[0])

    subject_files.sort()

    #subject_ids = [os.path.basename(os.path.dirname(s)) for s, _, _ in subject_files]
    #test, delete:
    temp = os.path.dirname(subject_files[0])


    subject_ids = [os.path.basename(temp)]
