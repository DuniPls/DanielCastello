...
import pandas as pd
from conversionUnique import timesync_from_cwa
from segment_and_calculate_features import segment_acceleration_and_calculate_features

model_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models_new")

model_paths = {
    100: os.path.join(model_folder, "healthy_3.0s_model_100.0hz.pickle"),
    #50: os.path.join(model_folder, "healthy_3.0s_model_50.0hzRESAMPLE.pickle"),
...
def complete_end_to_end_prediction(back_cwa, thigh_cwa, end_result_path, sampling_frequency=100, minutes_to_read_in_a_chunk=15):
    a = time()
...
def load_csv_and_extract_features(back_csv_path, thigh_csv_path, sampling_frequency, minutes_to_read_in_a_chunk):
    ...
    while True:
        try:
            ...
            this_windows_predictions = models[50].predict(boths_features)
...
if __name__ == "__main__":
    cwa_1 = os.path.join(os.path.dirname(os.path.abspath(__file__)), "/home/guest/Documents/HUNT4-data/50-100-Hz/raw/7050/7050_B.cwa")
    cwa_2 = os.path.join(os.path.dirname(os.path.abspath(__file__)), "/home/guest/Documents/HUNT4-data/50-100-Hz/raw/7050/7050_T.cwa")
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "timestamped_predictions_s=3.0_o=0.8_RESAMPLE.csv")
